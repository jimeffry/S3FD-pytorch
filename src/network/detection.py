...
class DetectIou(Function):
    ...
    def forward(self, loc_data, conf_data,iou_data, prior_data):
        """
        Args:
            loc_data: (tensor) Loc preds from loc layers
                Shape: [batch,num_priors*4]
        ...
        for i in range(num):
            boxes = decoded_boxes[i].clone()
            conf_scores = conf_preds[i].clone()
            iou_scores = iou_data[i].clone()
            for cl in range(1, self.num_classes):
                c_mask = conf_scores[cl].gt(self.conf_thresh)
                scores = conf_scores[cl][c_mask]
                iou_s = iou_scores[c_mask]
                if scores.numel() == 0:
                    continue
                l_mask = c_mask.unsqueeze(1).expand_as(boxes)
                boxes_ = boxes[l_mask].view(-1, 4)
                scores = scores.pow(0.5) * iou_s.pow(0.5)
                ids, count = nms(boxes_, scores, self.nms_thresh, self.nms_top_k)
                count = count if count < self.top_k else self.top_k
                output[i, cl, :count] = torch.cat((scores[ids[:count]].unsqueeze(1),boxes_[ids[:count]]), 1)
        return output
...
class Detect_demo(object):
    ...
    def __call__(self, loc_data, conf_data, prior_data):
        ...
        for i in range(num):
            ...
            # if count >0:
            box_score = [boxes_.detach().cpu().numpy(),scores.detach().cpu().numpy()] #[boxes_[ids[:count]].detach().numpy(),scores[ids[:count]].detach().numpy()]
