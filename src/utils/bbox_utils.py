...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t,soft_t, idx):
    ...
    # (Bipartite Matching)
    th1, th2, th3 = threshold  # _th1 = 0.1 ,_th2 = 0.35,_th3 = 0.5
    # sort_prior_overlap,sort_prior_idx = overlaps.sort(1,descending=True)
    # gt_prior = overlaps.gt(th2)
    # gt_num = gt_prior.sum(1)
    # gt_mean = gt_num.float().mean()
    # lt_idx = gt_num.lt(gt_mean.long())
    # gt_num.masked_fill_(lt_idx,gt_mean)
    # for ri in range(sort_prior_overlap.size()[0]):
    #     for li in range(gt_num[ri]):
    #         if sort_prior_overlap[ri,li] < th1:
    #             break
    #         elif overlaps[ri,sort_prior_idx[ri,li]] < th2:
    #             overlaps[ri,sort_prior_idx[ri,li]] = th2
    #print('gt_priors:',gt_num.size(),gt_num.numpy(),gt_mean.numpy())
...
def match_ssd(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    conf[best_truth_overlap < threshold[1]] = -1 # label in [0.3,0.5) ignored
    conf[best_truth_overlap < threshold[0]] = 0  # label as background
    loc = encode(matches, priors, variances)
    loc_t[idx] = loc    # [num_priors,4] encoded offsets to learn
    conf_t[idx] = conf  # [num_priors] top class label for each prior

def match_soft(threshold, truths, priors, variances, labels, loc_t, conf_t,soft_t, idx):
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
    ...
    best_prior_idx.squeeze_(1)
    best_prior_overlap.squeeze_(1)
    # for test for training only hold the best prior
    # best_truth_overlap = torch.zeros_like(best_truth_overlap)  
    best_truth_overlap.index_fill_(0, best_prior_idx, 2)  # ensure best prior
    # TODO refactor: index  best_prior_idx with long tensor
    # ensure every gt matches with its prior of max overlap
    for j in range(best_prior_idx.size(0)):
